...
export default function CheckoutModal({
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      // Segunda garantia de recebimento: envia a reserva por e-mail para o administrador.
      // A falha do e-mail não bloqueia a reserva nem o redirecionamento para o WhatsApp.
      const emailController = new AbortController();
      const emailTimeout = setTimeout(() => emailController.abort(), 5000);

      try {
        const emailResponse = await fetch("/api/notify-reservation", {
          method: "POST",
          headers: {
        ...
        if (!emailResponse.ok) {
          ...
          console.error("Falha ao enviar e-mail da reserva:", emailPayload?.error || emailResponse.statusText);
        }
      } catch (emailError) {

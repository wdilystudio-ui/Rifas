...
export default function Admin() {
  ...
  const [deletingPrizeImage, setDeletingPrizeImage] = useState(false);
...
setSelectedReservationIds((current) =>
  current.filter((id) => nextReservations.some((item) => item.id === id))
);
    } catch (err) {
      console.error(err);
  ...
  async function savePrizeImage(event) {
    ...
    if (prizeImageFile.size < 10 * 1024) {
      setPrizeImageMessage("❌ A imagem precisa ter no mínimo 10 KB.");
      return;
    }
  ...
  async function deletePrizeImage() {
    if (!prizeImageUrl) {
      setPrizeImageMessage("🔍 Nenhuma imagem do prêmio está salva no momento.");
      return;
    }

    const confirmed = window.confirm("Tem certeza que deseja apagar a imagem do prêmio? A apresentação voltará a mostrar o texto padrão do prêmio.");
    if (!confirmed) return;

    try {
      setDeletingPrizeImage(true);
      setPrizeImageMessage("");
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.message || "Erro ao apagar imagem do prêmio.");

      setPrizeImageUrl("");
      setPrizeImageFile(null);
      setPrizeImageMessage("✅ Imagem do prêmio apagada com sucesso.");
    } catch (err) {
      console.error(err);
      setPrizeImageMessage(err.message || "Erro ao apagar imagem do prêmio.");
    } finally {
      setDeletingPrizeImage(false);
    }
  }

  function toggleSelectedReservation(reservationId) {
    setSelectedReservationIds((current) => {
  ...
  function toggleAllReservations() {
    ...
    setSelectedReservationIds(reservations.map((item) => item.id));
  }
  ...
  async function bulkReservationAction(action) {
    ...
    const validItems = selectedItems
      .filter((item) => item.id && item.raffle_numbers?.id)
      .map((item) => ({
        reservationId: item.id,
        numberId: item.raffle_numbers.id
      }));
    ...
    if (!validItems.length) {
      alert("Nenhum número válido foi selecionado.");
      return;
    }

    if (action === "mark_paid") {
      const confirmPaid = window.confirm(
        `Deseja marcar ${validItems.length} número(s) selecionado(s) como pago(s)?`
      );
    ...
    if (action === "release_unpaid") {
      const confirmRelease = window.confirm(
        `Deseja liberar os números selecionados que ainda NÃO foram pagos? Números pagos não serão liberados por segurança.`
      );
    ...
    if (action === "delete_selected") {
      const confirmDelete = window.confirm(
        `Tem certeza que deseja apagar ${validItems.length} item(ns) selecionado(s)? Esta ação remove clientes pagantes selecionados e libera os números para outras pessoas.`
      );
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.message || "Erro ao executar ação em massa.");
      }

      alert(payload.message || "Ação realizada com sucesso.");
      setSelectedReservationIds([]);
      fetchReservations();
    } catch (err) {
      console.error(err);
      alert(err.message || "Erro ao executar ação em massa.");
    } finally {
  ...
  async function generateReceipt(item) {
    if (!item.group_id && !item.id && !item.customer_id) return;
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.message || "Erro ao gerar comprovante.");

      if (payload.receipt) {
        setReceiptData({
          customers: {
            name: payload.receipt.customer_name,
            phone: payload.receipt.customer_phone
          },
          group_id: payload.receipt.group_id || groupId,
          raffle_identifier: payload.receipt.raffle_identifier,
          all_numbers: payload.receipt.numbers,
          ticket_price: payload.receipt.ticket_price,
          total: payload.receipt.total,
          created_at: payload.receipt.created_at,
          generated_at: payload.receipt.generated_at
        });
        setShowReceipt(true);
      }
    } catch (err) {
      console.error(err);
      alert(err.message || "Erro ao gerar comprovante.");
    } finally {
    ...
    <main className="min-h-screen bg-slate-900 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <section className="bg-white rounded-3xl p-5 md:p-6 shadow-xl mb-6">
          <h2 className="text-2xl font-black text-slate-900">🔐 Segurança da senha</h2>
          <p className="text-slate-700 mt-1 mb-4">Altere a senha do administrador com hash seguro. {passwordHelp}</p>

          <form onSubmit={handleChangePassword} className="grid gap-3 md:grid-cols-[220px_1fr] md:items-center">
            ...
            <button className="md:col-span-1 rounded-xl bg-slate-900 text-white px-5 py-3 font-black hover:bg-slate-800 transition">Salvar</button>
            {passwordMessage && <p className={`md:col-span-2 text-sm font-bold border rounded-xl px-4 py-3 ${passwordMessage.startsWith("✅") ? "text-green-800 bg-green-100 border-green-200" : "text-red-800 bg-red-100 border-red-200"}`}>{passwordMessage}</p>}
          </form>
        ...
        <section className="bg-white rounded-3xl p-5 md:p-6 shadow-xl mb-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <h2 className="text-2xl font-black text-slate-900">🎰 Quantidade da Rifa</h2>
          ...
          <div className="mt-6">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-[220px_1fr] md:items-end">
              ...
              <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
                <button type="button" onClick={() => saveSettings()} disabled={savingSettings} className="rounded-xl bg-green-600 text-white px-5 py-3 text-sm font-black hover:bg-green-700 disabled:opacity-60 transition">
                  {savingSettings ? "Salvando..." : "Salvar"}
                </button>

                {settingsMessage && (
                  <p className={`text-sm font-bold border rounded-xl px-4 py-3 ${settingsMessage.startsWith("❌") ? "text-red-800 bg-red-100 border-red-200" : "text-green-800 bg-green-100 border-green-200"}`}>{settingsMessage}</p>
                )}
              </div>
            </div>
        ...
        <section className="bg-white rounded-3xl p-5 md:p-6 shadow-xl mb-6">
          <h2 className="text-2xl font-black text-slate-900">🎁 Imagem da apresentação do prêmio</h2>
          <p className="text-slate-700 mt-1 mb-4">Envie uma imagem JPG, PNG ou WEBP com no mínimo 10 KB e no máximo 5 MB. Essa imagem substituirá toda a apresentação inicial do prêmio.</p>

          {prizeImageUrl && (
            <div className="mb-4 rounded-2xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-xs font-black text-slate-700 mb-2 uppercase tracking-wider">Imagem atual</p>
              <img src={prizeImageUrl} alt="Imagem atual da apresentação" className="max-h-64 w-full object-contain rounded-xl bg-white" />
              <button
                type="button"
                onClick={deletePrizeImage}
                disabled={deletingPrizeImage || savingPrizeImage}
                className="mt-3 rounded-xl bg-red-600 text-white px-5 py-3 font-black hover:bg-red-700 disabled:opacity-60 transition"
              >
                {deletingPrizeImage ? "Apagando..." : "Apagar imagem do prêmio"}
              </button>
            </div>
          )}
          ...
          <form onSubmit={savePrizeImage} className="grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
            ...
            {prizeImageMessage && (
              <p className={`md:col-span-2 text-sm font-bold border rounded-xl px-4 py-3 ${prizeImageMessage.startsWith("❌") ? "text-red-800 bg-red-100 border-red-200" : "text-green-800 bg-green-100 border-green-200"}`}>{prizeImageMessage}</p>
            )}
          </form>
        </section>
        ...
        <div className="mb-6 bg-white rounded-2xl shadow-xl p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-xl font-black text-slate-900">Ações em massa</h2>
              <p className="text-sm text-slate-600 font-semibold">
                {selectedReservationIds.length
                  ? `${selectedReservationIds.length} número(s) selecionado(s).`
                  : "Selecione os números na tabela abaixo."}
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                disabled={!reservations.length || bulkActionLoading}
                className="rounded-lg bg-slate-900 text-white px-3 py-2 text-sm font-black hover:bg-slate-800 disabled:opacity-60 transition"
              >
                {selectedReservationIds.length === reservations.length && reservations.length > 0
                  ? "Limpar"
                  : "Todos"}
              </button>
              ...
              <button
                type="button"
                onClick={() => bulkReservationAction("mark_paid")}
                disabled={!selectedReservationIds.length || bulkActionLoading}
                className="rounded-lg bg-green-600 text-white px-3 py-2 text-sm font-black hover:bg-green-700 disabled:opacity-60 transition"
              >
                Pago
              </button>
              ...
              <button
                type="button"
                onClick={() => bulkReservationAction("release_unpaid")}
                disabled={!selectedReservationIds.length || bulkActionLoading}
                className="rounded-lg bg-amber-600 text-white px-3 py-2 text-sm font-black hover:bg-amber-700 disabled:opacity-60 transition"
              >
                Não pagos
              </button>
              ...
              <button
                type="button"
                onClick={() => bulkReservationAction("delete_selected")}
                disabled={!selectedReservationIds.length || bulkActionLoading}
                className="rounded-lg bg-red-700 text-white px-3 py-2 text-sm font-black hover:bg-red-800 disabled:opacity-60 transition"
              >
                Apagar
              </button>
            </div>
        ...
        {loading ? (
          ...
          <AdminTable
            ...
            onToggleSelected={toggleSelectedReservation}
            onToggleAll={toggleAllReservations}
            allSelected={reservations.length > 0 && selectedReservationIds.length === reservations.length}
            onMarkAsPaid={markAsPaid}
            onRelease={releaseReservation}
            onRefundPaid={refundPaidClient}
